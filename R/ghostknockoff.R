GhostKnockoff.prelim = \(cor.G, M=5, method="asdp", max.size=500){
  ...
  Normal_50Studies = matrix(stats::rnorm(n.G*M*50),n.G*M,50)
  P.each = matrix(0,n.G,n.G)
  if(length(temp.index) != 0) {
    Sigma = cor.G[temp.index,temp.index,drop=FALSE]
    SigmaInv = solve(Sigma) #invcov.shrink(Sigma,verbose=FALSE)
    ...
    if(sum(s) == 0) {
      ...
      V.each = Matrix::Matrix(Matrix::forceSymmetric(Sigma_k-diag_s))

      #random part of knockoff
      V = matrix(1,M,M) %x% V.each
      Matrix::diag(V) = Matrix::diag(V) + rep(s,M)
    ...
    Normal_50Studies[V.index,] = as.matrix(V.left %*% matrix(stats::rnorm(ncol(V.left)*50),ncol(V.left),50))
  ...
  Normal_50Studies[permute.V.index==1,] = matrix(stats::rnorm(sum(permute.index)*M*50),sum(permute.index)*M,50)

  return(list(P.each = as.matrix(P.each), V.left = V.left,
              Normal_50Studies = as.matrix(Normal_50Studies),
              permute.index = permute.index, M = M))
}

create.solve_sdp_M = \(Sigma, M=1, gaptol=1e-6, maxit=1000, verbose=FALSE) {
  # Check that covariance matrix is symmetric
  stopifnot(Matrix::isSymmetric(Sigma))
  # Convert the covariance matrix to a correlation matrix
  G = Matrix::cov2cor(Sigma)
  p = dim(G)[1]
  ...
  K=NULL
  K$s=p
  K$l=2*p #not sure if it should be changed - may be not as it is the dimension of the linear part.
...
#' data = cbind(x, y, z)
#' kocmi::construct_knockoff(data, 1, c(1,3))
#'
